...
describe("NoFlo Network", () => {
  ...
  describe("with an empty graph", () => {
    ...
    describe("with new node", () => {
      ...
      it("should have transmitted the node metadata to the process", () => {
        ...
        assert.strictEqual(
          typeof n.processes.Graph.component.metadata,
          "object",
        );
  ...
  describe("with a simple graph", () => {
    ...
    it("the ports of the processes should know the node names", () => {
      Object.keys(n.processes.Callback.component.inPorts.ports).forEach(
        (name) => {
      ...
      Object.keys(n.processes.Callback.component.outPorts.ports).forEach(
        (name) => {
    ...
    it("should emit a process-error when a component throws", () =>
      Promise.resolve()
        .then(() =>
          n.removeInitial({
            to: {
    ...
    describe("with a renamed node", () => {
      it("should have the process in a new location", () =>
        n.renameNode("Callback", "Func").then(() => {
          assert.strictEqual(typeof n.processes.Func, "object");
        }));
      it("shouldn't have the process in the old location", () => {
        assert.strictEqual(
          Object.keys(n.processes).includes("Callback"),
          false,
        );
      });
      ...
      it("should fail to rename with the old name", () =>
        n.renameNode("Callback", "Func").then(
          () => Promise.reject(new Error("Unexpected success")),
          (err) => {
            assert.strictEqual(Error.isError(err), true);
      ...
      it("should have informed the ports of their new node name", () => {
        Object.keys(n.processes.Func.component.inPorts.ports).forEach(
          (name) => {
        ...
        Object.keys(n.processes.Func.component.outPorts.ports).forEach(
          (name) => {
    ...
    describe("once stopped", () => {
      it("should be marked as stopped", () =>
        n.stop().then(() => {
          assert.equal(n.isStarted(), false);
        }));
    });
  ...
  describe("with an existing IIP", () => {
    ...
    it("should allow removing the IIPs", () =>
      Promise.resolve()
        .then(() =>
          n.removeInitial({
            to: {
      ...
      return noflo
        ...
        .then(
          ...
          (err) => {
            assert.strictEqual(Error.isError(err), true);
            assert.ok(
              err.message.includes("No process defined for outbound node"),
            );
          },
        );
    });
      ...
      return noflo
        ...
        .then(
          ...
          (err) => {
            assert.strictEqual(Error.isError(err), true);
            assert.ok(
              err.message.includes("No process defined for inbound node"),
            );
          },
        );
    });
